...
func (s *Service) handleStatusEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	if textFormat {
		...
		w.Write(formatStatusText(status, credentials, "CCM"))
		return
	}
...
func (s *Service) handleStatusStream(w http.ResponseWriter, r *http.Request, provider credentialProvider, userConfig *option.CCMUser, textFormat bool) {
	...
	if textFormat {
		...
		buf.Write(formatStatusText(last, s.visibleCredentials(provider, userConfig), "CCM"))
	} else {
	...
	for {
		...
		drained:
			...
			if textFormat {
				fmt.Fprintf(buf, "\n────── %s ──────\n\n", time.Now().Format("15:04:05"))
				buf.Write(formatStatusText(current, s.visibleCredentials(provider, userConfig), "CCM"))
			} else {
...
func (s *Service) visibleCredentials(provider credentialProvider, userConfig *option.CCMUser) []Credential {
